...
func TestGetDailyFinancialsByMarketSegment(t *testing.T) {
	req := client.NewGetDailyFinancialsByMarketSegmentRequest()
	...
	req.Body.BusinessDate = jonas_chorum.Date{time.Date(2025, 02, 23, 0, 0, 0, 0, time.UTC)}
	resp, err := req.Do()
	if err != nil {
		t.Error(err)
	}
